...
class HospitalSearchTab extends StatefulWidget {
  ...
  const HospitalSearchTab({super.key, required this.hospitalData, required this.primaryColor});
  @override
  State<HospitalSearchTab> createState() => _HospitalSearchTabState();
}

class _HospitalSearchTabState extends State<HospitalSearchTab>
  ...
  Future<void> _loadLocationData() async {
    try {
      ...
      setState(() { _locationData = json; _states = json.keys.toList()..sort(); _loadingLocation = false; });
    } catch (_) { setState(() => _loadingLocation = false); }
  }

  void _onStateChanged(String? state) {
    setState(() {
      _filterState = state; _filterDistrict = null; _cityCtrl.clear();
      if (state != null) {
    ...
    try {
      if (_searchMode == 0) {
        ...
        if (_cityCtrl.text.trim().isNotEmpty) {
          ...
          results = results.where((d) => (d['city'] as String? ?? '').toLowerCase().contains(q)).toList();
        }
  ...
  void _clearFilters() => setState(() { _filterState = null; _filterDistrict = null; _districts = []; _cityCtrl.clear(); });
  bool get _hasActiveFilters => _filterState != null || _filterDistrict != null || _cityCtrl.text.isNotEmpty;
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(child: FadeTransition(opacity: _fadeAnim, child: SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text('Find donors or blood banks', style: TextStyle(fontSize: 14, color: Colors.grey.shade500)),
        const SizedBox(height: 20),

        // Mode toggle — Donors / Blood Banks only
        Container(
          padding: const EdgeInsets.all(4),
          decoration: BoxDecoration(color: Colors.grey.shade100, borderRadius: BorderRadius.circular(14)),
          child: Row(children: [
            _buildToggle('Donors', 0, Icons.favorite_rounded, color),
            _buildToggle('Blood Banks', 1, Icons.local_hospital_rounded, color),
          ]),
        ),
        const SizedBox(height: 20),
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          const Text('Location Filter', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF1A1A2E))),
          Row(children: [
            if (_hasActiveFilters) ...[
              GestureDetector(onTap: _clearFilters,
            ...
            GestureDetector(
              ...
              child: AnimatedContainer(duration: const Duration(milliseconds: 200),
                  ...
                  child: Row(children: [
                    Icon(_showFilters ? Icons.expand_less : Icons.tune_rounded, size: 14,
        ...
        AnimatedCrossFade(
          ...
          firstChild: Container(
            ...
            decoration: BoxDecoration(color: color.withValues(alpha: 0.04), borderRadius: BorderRadius.circular(16),
                border: Border.all(color: color.withValues(alpha: 0.15))),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('State', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: Color(0xFF1A1A2E))),
              const SizedBox(height: 6),
              _loadingLocation ? Center(child: CircularProgressIndicator(color: color, strokeWidth: 2))
                  : DropdownButtonFormField<String>(
                  value: _filterState, isExpanded: true,
                  ...
                  onChanged: _onStateChanged, dropdownColor: Colors.white, menuMaxHeight: 250,
                  icon: Icon(Icons.keyboard_arrow_down_rounded, color: Colors.grey.shade400)),
              const SizedBox(height: 10),
              const Text('District', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: Color(0xFF1A1A2E))),
              const SizedBox(height: 6),
              IgnorePointer(ignoring: _filterState == null, child: Opacity(opacity: _filterState == null ? 0.5 : 1.0,
                  child: DropdownButtonFormField<String>(
                      value: _filterDistrict, isExpanded: true,
                      ...
                      dropdownColor: Colors.white, menuMaxHeight: 250,
                      icon: Icon(Icons.keyboard_arrow_down_rounded, color: Colors.grey.shade400)))),
              const SizedBox(height: 10),
              const Text('City (type manually)', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: Color(0xFF1A1A2E))),
              const SizedBox(height: 6),
              TextFormField(controller: _cityCtrl, style: const TextStyle(fontSize: 13),
        ...
        SizedBox(width: double.infinity, height: 50,
            child: ElevatedButton.icon(
                ...
                label: Text(_isSearching ? 'Searching...' : _searchMode == 0 ? 'Search Donors' : 'Search Blood Banks'),
        ...
        if (_results.isNotEmpty) ...[
          ...
          ..._results.asMap().entries.map((entry) => TweenAnimationBuilder<double>(
              ...
              child: _searchMode == 0 ? _buildDonorCard(entry.value, color) : _buildBankCard(entry.value, color))),
  ...
  Widget _buildToggle(String label, int index, IconData icon, Color color) {
  ...
  InputDecoration _inputDeco(Color color) => InputDecoration(
      filled: true, fillColor: Colors.white,
      border: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide(color: Colors.grey.shade200)),
      enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide(color: Colors.grey.shade200)),
      focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide(color: color, width: 1.5)),
  ...
  Widget _buildChip(String label, Color color, VoidCallback onRemove) => Container(
      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
      decoration: BoxDecoration(color: color.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(20),
          border: Border.all(color: color.withValues(alpha: 0.3))),
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        Icon(Icons.location_on_rounded, size: 12, color: color), const SizedBox(width: 4),
        Text(label, style: TextStyle(fontSize: 12, color: color, fontWeight: FontWeight.w500)), const SizedBox(width: 4),
        GestureDetector(onTap: onRemove, child: Icon(Icons.close_rounded, size: 14, color: color)),
      ]));

  Widget _buildDonorCard(Map<String, dynamic> d, Color color) => Container(
      ...
      child: Row(children: [
        Container(width: 46, height: 46, decoration: BoxDecoration(shape: BoxShape.circle, color: color.withValues(alpha: 0.1)),
            child: Center(child: Text(d['blood_group'] ?? '?', style: TextStyle(color: color, fontWeight: FontWeight.bold, fontSize: 14)))),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (d['age'] != null) Text('Age ${d['age']}', style: TextStyle(color: Colors.grey.shade400, fontSize: 11)),
        ])),
  ...
  Widget _buildBankCard(Map<String, dynamic> d, Color color) => Container(
      margin: const EdgeInsets.only(bottom: 12), padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16),
          boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.05), blurRadius: 10, offset: const Offset(0, 4))]),
      child: Row(children: [
        ...
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(d['bank_name'] ?? 'Blood Bank', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14, color: Color(0xFF1A1A2E))),
          Text([d['city'], d['district'], d['state']].where((e) => e != null && e.toString().isNotEmpty).join(', '),
              style: TextStyle(color: Colors.grey.shade500, fontSize: 12)),
