...
class _HospitalSearchTabState extends State<HospitalSearchTab>
  ...
  // Kept in sync with donor_search_tab.dart's badge tiers so the badge
  // shown here always matches what the donor sees on their own profile.
      ...
      if (_searchMode == 0) {
        ...
        if (_nameSearchCtrl.text.trim().isNotEmpty) {
          ...
          results = results.where((d) =>
              (d['bank_name']?.toString().toLowerCase() ?? '').contains(keyword)).toList();
        }
  ...
  // Donors with a fresh last_lat/last_lng get their own precise pin instead
  // of being clustered purely by city text.
  ...
  void _clearFilters() => setState(() {
    ...
    _cityCtrl.clear(); _nameSearchCtrl.clear();
    _results.clear(); _clusters.clear(); _hasSearched = false;
  });
  ...
  Future<void> _messageNumber(String? phone) async {
    ...
    try {
      final launched = await launchUrl(uri, mode: LaunchMode.externalApplication);
      if (!launched && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Could not open Messages app')));
      }
    } catch (_) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Could not open Messages app')));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(child: FadeTransition(opacity: _fadeAnim, child: SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (_searchMode != 0) ...[
          const Text('Search by Name', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF1A1A2E))),
        ...
        AnimatedCrossFade(
          ...
          firstChild: Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 6),
              TextFormField(
                controller: _cityCtrl, style: const TextStyle(fontSize: 13),
                decoration: InputDecoration(
        ...
        if (_results.isNotEmpty) ...[
          ...
          if (_showMap)
            ...
            ..._results.asMap().entries.map((entry) {
              ...
              return TweenAnimationBuilder<double>(
                ...
                child: _searchMode == 0 ? _buildDonorCard(d, color) : _buildBankCard(d, color),
              );
            }),
          ...
          Center(child: Padding(padding: const EdgeInsets.all(30),
              child: Column(children: [
                ...
                const SizedBox(height: 12),
                Text(_searchMode == 0 ? 'No donors found' : 'No blood bank found',
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ...cluster.donors.map((d) => _searchMode == 0 ? _buildDonorCard(d, color) : _buildBankCard(d, color)),
        ]),
      ),
    );
  }
