...
import 'tabs/blood_bank_home_tab.dart';
import 'tabs/blood_bank_stock_tab.dart';
import 'tabs/blood_bank_search_tab.dart';
import 'tabs/blood_bank_profile_tab.dart';
...
class _BloodBankDashboardState extends State<BloodBankDashboard> with TickerProviderStateMixin {
  int _currentIndex = 0;
  ...
  Future<void> _fetchBankData() async {
    try {
      ...
      if (uid == null) return;
      final doc = await FirebaseFirestore.instance.collection('blood_banks').doc(uid).get();
      if (doc.exists) setState(() { _bankData = doc.data(); _isLoading = false; });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: PageView(
        ...
        children: [
          BloodBankHomeTab(bankData: _bankData, primaryColor: _primaryColor, onDataUpdated: _fetchBankData),
          BloodBankStockTab(bankData: _bankData, primaryColor: _primaryColor, onDataUpdated: _fetchBankData),
          BloodBankSearchTab(bankData: _bankData, primaryColor: _primaryColor),
          BloodBankProfileTab(bankData: _bankData, primaryColor: _primaryColor, onDataUpdated: _fetchBankData),
        ],
      ),
