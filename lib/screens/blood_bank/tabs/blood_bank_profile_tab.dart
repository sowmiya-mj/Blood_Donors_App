...
import '../blood_bank_edit_profile_screen.dart';
import '../../auth/role_selection_screen.dart';

class BloodBankProfileTab extends StatefulWidget {
  final Map<String, dynamic>? bankData;
  final Color primaryColor;
  final VoidCallback onDataUpdated;
  const BloodBankProfileTab({super.key, required this.bankData, required this.primaryColor, required this.onDataUpdated});
  @override
  State<BloodBankProfileTab> createState() => _BloodBankProfileTabState();
}

class _BloodBankProfileTabState extends State<BloodBankProfileTab> with SingleTickerProviderStateMixin {
  ...
  Future<void> _editProfile() async {
    final updated = await Navigator.push<bool>(context, MaterialPageRoute(
        builder: (_) => BloodBankEditProfileScreen(
            bankData: widget.bankData, primaryColor: widget.primaryColor)));
  ...
  Widget build(BuildContext context) {
    ...
    final name = data?['bank_name'] ?? 'Blood Bank';
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              child: Column(children: [
                ...
                Container(
                    ...
                    child: Row(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      Text('Registered Blood Bank', style: TextStyle(color: Colors.white.withValues(alpha: 0.9), fontSize: 12)),
                    ])),
              ]),
            ),
            ...
            Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const Text('Blood Bank Info', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF1A1A2E))),
                const SizedBox(height: 12),
                _buildInfoCard([
                  _Info(Icons.business_outlined, 'Blood Bank Name', name),
                ...
                const Text('Location', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF1A1A2E))),
                const SizedBox(height: 12),
                _buildInfoCard([
                  _Info(Icons.location_city_outlined, 'City', city),
                  _Info(Icons.map_outlined, 'District', district),
                  _Info(Icons.flag_outlined, 'State', state),
                ], color),

                const SizedBox(height: 20),

                const Text('Account', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF1A1A2E))),
                const SizedBox(height: 12),
  ...
  Widget _buildInfoCard(List<_Info> items, Color color) {
    return Container(
